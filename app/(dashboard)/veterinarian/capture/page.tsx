...
import {
  Camera, Thermometer, Weight, Heart, Activity, Clock, AlertCircle
} from 'lucide-react';
import { format } from 'date-fns';

// Fetch today's confirmed appointments ready for capture
...
export default function CapturePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await supabase
        .from('appointments')
        .update({ appointment_status: 'checked-in' })
        .eq('id', selectedAppt.id);

      mutate('capture-queue');
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto h-[calc(100vh-100px)] flex flex-col">
      <div>
        <h1 className="text-3xl font-bold text-foreground">Neuter <span className="text-lg font-normal text-muted-foreground">(Kapon)</span></h1>
        <p className="text-muted-foreground">Record patient vitals and initial assessment</p>
      </div>
            ...
            <Card className="h-full border-t-4 border-t-primary">
              ...
              <CardContent className="p-6">
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {/* Priority Level & Chief Complaint */}
