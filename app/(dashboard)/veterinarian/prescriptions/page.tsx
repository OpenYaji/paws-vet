...
export default function PrescriptionsPage() {
  ...
  // Filter Logic
  const filteredList = Array.isArray(prescriptions)
    ...
    <div className="space-y-6 max-w-7xl mx-auto p-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage and issue medical prescriptions
          </p>
        </div>
        <IssuePrescription
      ...
      {isLoading ? (
        ...
        <div className="text-center py-12 bg-muted/50 rounded-lg border border-dashed">
          ...
          <p className="text-muted-foreground">
            Issue a new prescription to see it here.
          </p>
        </div>
      ) : (
            ...
            >
              <CardContent className="p-5 flex flex-col md:flex-row justify-between gap-4 items-start md:items-center">
                ...
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-2">
                      <span className="font-medium">Sig:</span> {rx.frequency}{" "}
                      for {rx.duration}
                    </p>
                  </div>
                </div>
                ...
                <div className="text-right flex flex-col items-end gap-2">
                  <Badge
                    variant={rx.dispensed_date ? "default" : "secondary"}
                    className={
                      rx.dispensed_date ? "bg-green-600" : "bg-amber-500"
                    }
                  >
                  ...
                  {rx.dispensed_date && (
                    <span className="text-xs text-muted-foreground">
                      Dispensed:{" "}
                      {new Date(rx.dispensed_date).toLocaleDateString()}
                    </span>
                  )}
