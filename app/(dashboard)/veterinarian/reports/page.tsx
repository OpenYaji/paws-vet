...
export default function VetReportsPage() {
  ...
  const maxByDay = apptReport ? Math.max(...Object.values(apptReport.by_day), 1) : 1;
  const maxByType = apptReport ? Math.max(...Object.values(apptReport.by_type), 1) : 1;
  const maxByStatus = apptReport ? Math.max(...Object.values(apptReport.by_status), 1) : 1;
        ...
        <Tabs defaultValue="appointments">
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  <StatCard
                    ...
                    value={(apptReport.by_status?.cancelled ?? 0) + (apptReport.by_status?.no_show ?? 0)}
                    sub={`this ${period}`}
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <Card>
                    ...
                    <CardContent className="space-y-2">
                      {Object.entries(apptReport.by_day).map(([day, count]) => (
                        <BarRow key={day} label={day} value={count} max={maxByDay} />
                      ))}
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm">By Appointment Type</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2">
                      {Object.keys(apptReport.by_type).length === 0 ? (
                        <p className="text-sm text-muted-foreground">No data</p>
                      ) : (
                        Object.entries(apptReport.by_type).map(([type, count]) => (
                          <BarRow key={type} label={type} value={count} max={maxByType} color="bg-blue-500" />
                        ))
                      )}
                    </CardContent>
                  </Card>
                    ...
                    {vetReport.medical_records.top_diagnoses.length === 0 ? (
                      ...
                      vetReport.medical_records.top_diagnoses.map(({ name, count }) => (
                        <BarRow
                          ...
                          label={name}
                          value={count}
                          max={vetReport.medical_records.top_diagnoses[0]?.count ?? 1}
            ...
            {vetLoading ? (
              ...
              <>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  <StatCard icon={<Pill className="h-5 w-5" />} label="Total Prescriptions" value={vetReport.prescriptions.total} />
                  {Object.entries(vetReport.prescriptions.by_status).map(([status, count]) => (
                    <StatCard key={status} icon={<Pill className="h-5 w-5" />} label={status} value={count} />
                  ))}
                </div>
                <Card>
                  <CardHeader className="pb-2">
                    ...
                    {vetReport.prescriptions.top_medications.length === 0 ? (
                      ...
                      vetReport.prescriptions.top_medications.map(({ name, count }) => (
                        <BarRow
                          ...
                          label={name}
                          value={count}
                          max={vetReport.prescriptions.top_medications[0]?.count ?? 1}
                          color="bg-violet-500"
                        />
                      ))
                    )}
          ...
          <TabsContent value="vaccinations" className="space-y-5 mt-5">
            {vetLoading ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <StatCard icon={<Syringe className="h-5 w-5" />} label="Total Records" value={vetReport.vaccinations.total} />
                  <StatCard
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        {vetReport.vaccinations.overdue.map((v, i) => (
                          <div key={i} className="flex items-center justify-between text-sm">
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground">{v.vaccine_name}</p>
                            </div>
                  ...
                  <Card className={vetReport.vaccinations.overdue.length > 0 ? "" : "md:col-span-2"}>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-sm">Most Administered Vaccines</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-2">
                      {vetReport.vaccinations.top_vaccines.length === 0 ? (
                        <p className="text-sm text-muted-foreground">No vaccination data yet.</p>
                      ) : (
                        vetReport.vaccinations.top_vaccines.map(({ name, count }) => (
                          <BarRow
                            key={name}
                            label={name}
                            value={count}
                            max={vetReport.vaccinations.top_vaccines[0]?.count ?? 1}
                            color="bg-teal-500"
                          />
                        ))
                      )}
