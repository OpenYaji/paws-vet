...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('Triage API - Found', data?.length || 0, 'in_progress appointments');
    
    // Log all checked_in_at times for debugging
    if (data && data.length > 0) {
      data.forEach((appt: any) => {
        console.log('Appointment', appt.id, '- Status:', appt.appointment_status, 'Checked in at:', appt.checked_in_at);
      });
    }
    ...
    console.log('Triage API - Excluded appointment IDs (already triaged):', Array.from(triageCompletedIds));
    ...
    const filteredData = (data || []).filter((appt: any) => {
      ...
      if (appt.checked_in_at) {
        ...
        const isToday = checkedInDate >= startOfDay && checkedInDate <= endOfDay;
        console.log('Appointment', appt.id, '- Checked in:', checkedInDate.toISOString(), 'Is today?', isToday);
        return isToday;
      }
      
      // If no checked_in_at but status is in_progress, include it
      console.log('Appointment', appt.id, '- No checked_in_at, including anyway');
      return true;
    });

    console.log('Triage API - Final queue:', filteredData.length, 'patients waiting');
