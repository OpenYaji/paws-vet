...
CREATE TABLE public.client_profiles (
  ...
  zip_code character varying CHECK (zip_code::text ~ '^[0-9]{4,5}$'::text),
...
CREATE TABLE public.invoice_line_items (
  ...
  created_at timestamp with time zone DEFAULT now(),
  item_type character varying CHECK (item_type::text = ANY (ARRAY['service'::character varying, 'product'::character varying]::text[])),
  product_id uuid,
...
CREATE TABLE public.invoices (
  ...
  deleted_by uuid,
  walk_in_customer_name character varying,
...
CREATE TABLE public.quarantine_pets (
  ...
  status USER-DEFINED,
  CONSTRAINT quarantine_pets_pkey PRIMARY KEY (id),
  CONSTRAINT quarantine_pets_pet_id_fkey FOREIGN KEY (pet_id) REFERENCES public.pets(id)
);
...
CREATE TABLE public.triage_records (
  ...
  triage_level text DEFAULT 'Non-Urgent'::text,
  chief_complaint text,
  notes text,
  created_by uuid,
  created_at timestamp with time zone DEFAULT now(),
  CONSTRAINT triage_records_pkey PRIMARY KEY (id),
  CONSTRAINT triage_records_appointment_id_fkey FOREIGN KEY (appointment_id) REFERENCES public.appointments(id),
  CONSTRAINT triage_records_pet_id_fkey FOREIGN KEY (pet_id) REFERENCES public.pets(id),
  CONSTRAINT triage_records_created_by_fkey FOREIGN KEY (created_by) REFERENCES auth.users(id)
);
